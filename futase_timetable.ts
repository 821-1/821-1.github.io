interface futase_TimeTableRow {
    id: number;
    type: number;
    depart: string;
    arriveBT: string | null;
    arriveSta: string | null;
    arriveTenjin: string|null;
}

const futase_timeTable: futase_TimeTableRow[]=[
...
if (futase_timeTable.length > 0) {
    ...
    const keys: Array<keyof typeof futase_timeTable[0]> = ['type','depart', 'arriveBT', 'arriveSta','arriveTenjin'];
